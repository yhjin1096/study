...
원본 PDF (ref/)
        ...
        ├── python3 _study_kit/tools/extract_images_bbox.py   ← 그림 10개 (캡션 없음)
        ▼
노트 .md (원본, 손으로 작성)
   +  tools/widgets/*.html (Canvas 위젯 12개)
   +  images/*.png (PDF 에서 뽑은 그림 10개)
   +  tools/widgets/_vm_helper.js → inject_helper.py 로 위젯에 주입
...
> **이 스터디는 그림이 10개이고 캡션은 하나뿐이다** (p13 의 "Figure 1"). 나머지 9개는
> 캡션이 없으므로 `extract_images_bbox.py` 를 쓴다. `extract_figures.py` 는 쓰지 않는다.
> 자세한 이유는 `kit.conf` 의 캡션 항목과 `README.md` ② 를 볼 것.
...
이 문서는 색을 **세 가지** 쓴다. 파랑 `#197fb2` 는 산문 강조, 빨강 `#ff0000` 과
파랑 `#0000ff` 는 **수식 안**이다. 전사 전에 뽑아 놓고 옆에 두고 쓴다.

```bash
python3 _study_kit/tools/dump_colored.py --count          # 개수만 — 전사 후 대조용
python3 _study_kit/tools/dump_colored.py -c prose         # 산문 강조만
python3 _study_kit/tools/dump_colored.py -c math          # 수식 안의 색만
python3 _study_kit/tools/dump_colored.py -p 14 -c m-red   # 14쪽의 빨강
```
...
위젯 12개가 `window.VM` 을 공유한다. 원본은 `tools/widgets/_vm_helper.js` 한 벌이고,
위젯 파일에는 `//%VM%` 표시만 둔다.
