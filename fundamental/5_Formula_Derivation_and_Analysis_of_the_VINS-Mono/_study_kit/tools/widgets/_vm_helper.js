// window.VM — 이 스터디의 위젯 12개가 공유하는 계산 라이브러리
//
//   앞부분은 4_On-manifold_preintegration 스터디의 window.PI 를 그대로 가져온 것이다
//   (SO(3) 지수·로그, 오른쪽 자코비안, 3D 씬, 난수, 수치미분, 2D 플롯).
//   뒷부분("여기부터 VINS-Mono 스터디에서 더한 것")이 이 문서 전용이다 —
//   쿼터니언 대수와 Ω_L/Ω_R(NOMENCLATURE), 식 (9) mid-point preintegration,
//   식 (37)(38) F·G 와 (27)(28) P·J 전파, 식 (29) bias 1차 보정,
//   식 (60) residual, 초기화 선형시스템, 식 (79)~(82) Schur complement.
//
//   쿼터니언은 [w, x, y, z] 순서의 길이 4 배열이다 (원문 표기와 같다).
//   중력은 원문 NOMENCLATURE 대로 g^w = [0,0,+9.8] 이다 — 부호에 주의.
//
//   위젯 파일에는 `//%VM%` 한 줄만 두고 tools/inject_helper.py 로 채워 넣는다.
//   검산 결과는 _study_kit/README.md 의 표를 볼 것.

if (!window.VM) window.VM = (function () {
    // ---- 일반 행렬 ------------------------------------------------------
...
return { zeros: zeros, eye: eye, mm: mm, mT: mT, madd: madd, msub: msub, mscale: mscale,
             ...
             OmegaL: OmegaL, OmegaR: OmegaR, q2R: q2R, R2q: R2q,
