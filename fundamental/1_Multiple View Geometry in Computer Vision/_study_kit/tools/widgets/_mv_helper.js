// ============================================================================
//  window.MV — Notes on Multiple View Geometry in Computer Vision 위젯 공용 헬퍼
//
//  헬퍼 계보:  LG(Lie Theory) → PI(Preintegration) → VM(VINS-Mono)
//              → EJ(Errors and Jacobians) → IC(ICP) → MV(여기)
//
//  위쪽 대부분은 물려받은 것이다 (SO(3)/SE(3), 쿼터니언, 고유분해·SVD,
//  수치미분, 선형대수, 2D 플롯). 이 스터디에서 더한 것은 파일 맨 아래
//  "Multiple View Geometry 스터디에서 더한 것" 구획에 모여 있다:
//    · 동차좌표 · 점/직선 duality · conic (식 20~33)
//    · 변환 계층과 분해 (식 34~39), affine/metric rectification (식 40~78)
//    · 카메라 행렬 P 조립·분해·해부 (식 79~116), DLT (식 140~156)
//    · IAC ω 와 소실점 (식 171~196)
//    · F·E, 8점 알고리즘, 카메라 4해 (식 202~275)
//    · 삼각측량 (식 316~341), 평면 homography (식 342~384)
//    · trifocal tensor 와 transfer (식 401~456)
// ============================================================================
...
window.MV = (function () {
    ...
    // ══ 여기부터 Multiple View Geometry 스터디에서 더한 것 ═══════════════

    // ---- 동차좌표 기본 -----------------------------------------------------
    function hnorm(x) {                                      // 마지막 항으로 나눈다 (식 3)
      var w = x[x.length-1];
      if (Math.abs(w) < 1e-300) return x.slice();            // 무한원점은 그대로
      return x.map(function (v) { return v/w; });
    }
    function h2e(x) { var y = hnorm(x); return y.slice(0, y.length-1); }
    function e2h(x) { return x.concat([1]); }
    function join(a, b) { return cross(a, b); }              // 식 (13) 두 점 → 직선
    function meet(a, b) { return cross(a, b); }              // 식 (11) 두 직선 → 점
    function onLine(x, l) { return dot(x, l); }              // 식 (9)
    // 점 x 와 직선 l 사이의 대수적/기하적 거리
    function ptLineDist(x, l) {
      var xh = hnorm(x);
      return Math.abs(dot(xh, l))/Math.hypot(l[0], l[1]);
    }

    // ---- 널 벡터 / 분해 ----------------------------------------------------
    // A 의 가장 작은 특이값에 대응하는 오른쪽 특이벡터 (Ax = 0 의 최소제곱 해)
    function nullvec(A) {
      var n = A[0].length;
      var e = eigSym(mm(mT(A), A));                          // 고유값 내림차순
      var v = [];
      for (var i = 0; i < n; i++) v.push(e.V[i][n-1]);
      return v;
    }
    function nullvecLeft(A) { return nullvec(mT(A)); }
    // RQ 분해 (3×3):  M = R_upper · Q_orth.  카메라 P → K, R 분해에 쓴다 (식 95)
    function rq3(M) {
      function givens(a, b) { var r = Math.hypot(a, b); return r < 1e-300 ? [1, 0] : [a/r, -b/r]; }
      var A = M.map(function (r) { return r.slice(); }), Q = eye(3), c, s, G;
      // (3,2) 를 0 으로
      var g = givens(A[2][2], A[2][1]); c = g[0]; s = g[1];
      G = [[1,0,0],[0,c,-s],[0,s,c]];
      A = mm(A, G); Q = mm(mT(G), Q);
      // (3,1) 을 0 으로
      g = givens(A[2][2], -A[2][0]); c = g[0]; s = g[1];
      G = [[c,0,s],[0,1,0],[-s,0,c]];
      A = mm(A, G); Q = mm(mT(G), Q);
      // (2,1) 을 0 으로
      g = givens(A[1][1], A[1][0]); c = g[0]; s = g[1];
      G = [[c,-s,0],[s,c,0],[0,0,1]];
      A = mm(A, G); Q = mm(mT(G), Q);
      // K 의 대각을 양수로 (부호 모호성 제거)
      var D = eye(3);
      for (var i = 0; i < 3; i++) D[i][i] = A[i][i] < 0 ? -1 : 1;
      A = mm(A, D); Q = mm(D, Q);
      return { R: A, Q: Q };                                 // R = 상삼각(K), Q = 직교
    }

    // ---- Conic (식 20~33) --------------------------------------------------
    function conicFrom5(pts) {                               // 식 (23)(24)
      var A = pts.slice(0, 5).map(function (p) {
        var q = hnorm(p);
        return [q[0]*q[0], q[0]*q[1], q[1]*q[1], q[0], q[1], 1];
      });
      var c = nullvec(A);
      return [[c[0], c[1]/2, c[3]/2], [c[1]/2, c[2], c[4]/2], [c[3]/2, c[4]/2, c[5]]];
    }
    function conicTangent(C, x) { return mv(C, x); }         // 식 (25)
    function conicFromLines(l, m) {                          // 식 (26)
      var A = zeros(3, 3);
      for (var i = 0; i < 3; i++) for (var j = 0; j < 3; j++) A[i][j] = l[i]*m[j] + m[i]*l[j];
      return A;
    }
    function adjugate3(M) {                                  // 식 (28) C* = adj(C)
      var A = zeros(3, 3);
      for (var i = 0; i < 3; i++) for (var j = 0; j < 3; j++) {
        var r = [0,1,2].filter(function (k) { return k !== i; });
        var c = [0,1,2].filter(function (k) { return k !== j; });
        var d = M[r[0]][c[0]]*M[r[1]][c[1]] - M[r[0]][c[1]]*M[r[1]][c[0]];
        A[j][i] = ((i+j) % 2 ? -1 : 1)*d;                    // 전치까지 포함
      }
      return A;
    }
    function conicPoints(C, n) {                             // 시각화용 — conic 위의 점들
      n = n || 200;
      var out = [];
      // x = (cos t, sin t, w) 형태로 w 를 풀어 곡선을 얻는다
      for (var i = 0; i < n; i++) {
        var t = i/n*2*Math.PI, cx = Math.cos(t), cy = Math.sin(t);
        var a = C[2][2];
        var b = 2*(C[0][2]*cx + C[1][2]*cy);
        var c2 = C[0][0]*cx*cx + 2*C[0][1]*cx*cy + C[1][1]*cy*cy;
        // a w² + b w + c = 0  (여기서 점은 (cx, cy, w) 가 아니라 (cx/w, cy/w))
        var disc = b*b - 4*a*c2;
        if (disc < 0) continue;
        var sq = Math.sqrt(disc);
        [(-b + sq)/(2*a), (-b - sq)/(2*a)].forEach(function (w) {
          if (isFinite(w) && Math.abs(w) > 1e-9) out.push([cx/w, cy/w]);
        });
      }
      return out;
    }
    ...
    // ---- Homography (식 34~50) --------------------------------------------
    function normalizePts(pts) {                             // 식 (305) Hartley 정규화
      var n = pts.length, cx = 0, cy = 0, i;
      var q = pts.map(hnorm);
      for (i = 0; i < n; i++) { cx += q[i][0]; cy += q[i][1]; }
      cx /= n; cy /= n;
      var d = 0;
      for (i = 0; i < n; i++) d += Math.hypot(q[i][0]-cx, q[i][1]-cy);
      d /= n;
      var s = d < 1e-300 ? 1 : Math.SQRT2/d;
      var T = [[s, 0, -s*cx], [0, s, -s*cy], [0, 0, 1]];
      return { T: T, pts: q.map(function (p) { return mv(T, p); }) };
    }
    // 4점 이상 → H.  normalize 를 끄면 정규화 없이 푼다 (Algorithm 7.1 의 대조용)
    function dltH(src, dst, normalize) {
      var S = (normalize === false) ? { T: eye(3), pts: src.map(hnorm) } : normalizePts(src);
      var D = (normalize === false) ? { T: eye(3), pts: dst.map(hnorm) } : normalizePts(dst);
      var A = [];
      for (var i = 0; i < S.pts.length; i++) {
        var x = S.pts[i], y = D.pts[i];
        A.push([0, 0, 0, -y[2]*x[0], -y[2]*x[1], -y[2]*x[2], y[1]*x[0], y[1]*x[1], y[1]*x[2]]);
        A.push([y[2]*x[0], y[2]*x[1], y[2]*x[2], 0, 0, 0, -y[0]*x[0], -y[0]*x[1], -y[0]*x[2]]);
      }
      var h = nullvec(A);
      var Hn = [[h[0],h[1],h[2]],[h[3],h[4],h[5]],[h[6],h[7],h[8]]];
      return mm(minv(D.T), mm(Hn, S.T));
    }
    // 식 (39) 사영변환 분해  H = H_S H_A H_P
    //   H_S H_A H_P = [[ sRK + t vᵀ ,  t·v ], [ vᵀ , v ]]
    //   원문 (39) 는 오른쪽 위를 t 로 적는데 이는 v = 1 로 정규화한 경우다.
    //   여기서도 H 를 v = 1 로 먼저 정규화한 뒤 분해한다.
    function decomposeH(Hin) {
      var v3 = Hin[2][2];
      var H = Math.abs(v3) > 1e-300 ? mscale(Hin, 1/v3) : Hin;
      var v = [H[2][0], H[2][1]];
      var t = [H[0][2], H[1][2]];
      var A = [[H[0][0], H[0][1]], [H[1][0], H[1][1]]];
      // A − t vᵀ = sRK   (R 회전, K 상삼각 det 1)
      var Ap = [[A[0][0] - t[0]*v[0], A[0][1] - t[0]*v[1]],
                [A[1][0] - t[1]*v[0], A[1][1] - t[1]*v[1]]];
      var dA = Ap[0][0]*Ap[1][1] - Ap[0][1]*Ap[1][0];
      var s = Math.sqrt(Math.abs(dA));                        // det(sRK) = s²
      var B = mscale(Ap, 1/Math.max(1e-300, s));              // B = R K
      var q = qr2(B);                                         // B = Q U
      return { s: s, R: q.Q, K: q.U, t: t, v: v, vv: 1 };
    }
    // 2×2 QR:  M = Q U  (Q 회전, U 상삼각).  Givens 한 번이면 된다.
    function qr2(M) {
      var r = Math.hypot(M[0][0], M[1][0]);
      var c = r < 1e-300 ? 1 : M[0][0]/r, sn = r < 1e-300 ? 0 : M[1][0]/r;
      var Q = [[c, -sn], [sn, c]];
      var U = mm(mT(Q), M);
      U[1][0] = 0;
      return { Q: Q, U: U };
    ...
    // 2×2 RQ:  M = U Q
    function rq2(M) {
      var r = Math.hypot(M[1][1], M[1][0]);
      var c = r < 1e-300 ? 1 : M[1][1]/r, sn = r < 1e-300 ? 0 : M[1][0]/r;
      var G = [[c, -sn], [sn, c]];
      var K = mm(M, G);
      return { K: [[K[0][0], K[0][1]], [0, K[1][1]]], R: mT(G) };
    }
    // 식 (48)(50) affine rectification:  소실선 l' 을 l∞ 로 보낸다
    function affineRectifyH(lp) {
      return [[1, 0, 0], [0, 1, 0], [lp[0], lp[1], lp[2]]];
    }
    // 식 (73)~(78) metric rectification: 직교 직선 쌍 두 개로 S = KKᵀ 를 푼다
    function metricRectifyH(pairs) {
      // (l1 l2) S (m1 m2)ᵀ = 0,  S = [[s11, s12],[s12, s22]]
      var A = pairs.map(function (pr) {
        var l = pr[0], m = pr[1];
        return [l[0]*m[0], l[0]*m[1] + l[1]*m[0], l[1]*m[1]];
      });
      var s = nullvec(A);
      var S = [[s[0], s[1]], [s[1], s[2]]];
      if (S[0][0] < 0) S = mscale(S, -1);
      var e = eigSym(S);
      var K2 = mm(e.V, [[Math.sqrt(Math.max(1e-300, e.w[0])), 0],
                        [0, Math.sqrt(Math.max(1e-300, e.w[1]))]]);
      var r = rq2(K2);                                       // K2 = K · Q
      var K = r.K;
      var H = [[K[0][0], K[0][1], 0], [K[1][0], K[1][1], 0], [0, 0, 1]];
      return { H: minv(H), K: K, S: S };
    }

    // ---- 카메라 행렬 (식 79~116) -------------------------------------------
    function Kmat(fx, fy, cx, cy, s) {
      return [[fx, s || 0, cx], [0, fy, cy], [0, 0, 1]];
    }
    function camP(K, R, t) {                                 // 식 (93) P = K[R|t]
      var P = zeros(3, 4), KR = mm(K, R), Kt = mv(K, t);
      for (var i = 0; i < 3; i++) {
        for (var j = 0; j < 3; j++) P[i][j] = KR[i][j];
        P[i][3] = Kt[i];
      }
      return P;
    }
    function camM(P) { return [P[0].slice(0,3), P[1].slice(0,3), P[2].slice(0,3)]; }
    function camP4(P) { return [P[0][3], P[1][3], P[2][3]]; }
    function camCenter(P) {                                  // 식 (97) PC = 0
      return nullvec(P);
    }
    function camCenterFinite(P) {                            // C̃ = −M⁻¹p₄
      return sc(mv(minv(camM(P)), camP4(P)), -1);
    }
    // 식 (95) P → K, R, t  (RQ 분해)
    function decomposeP(P) {
      var M = camM(P), r = rq3(M);
      var K = r.R, R = r.Q;
      if (det3(R) < 0) { R = mscale(R, -1); }
      var sc2 = K[2][2];
      K = mscale(K, 1/sc2);
      var C = camCenterFinite(P);
      return { K: K, R: R, C: C, t: sc(mv(R, C), -1) };
    }
    function principalPlane(P) { return P[2].slice(); }      // 식 (102)(104)
    function principalPoint(P) {                             // 식 (105)(106)
      var M = camM(P), m3 = M[2];
      return mv(M, m3);
    }
    function principalAxis(P) {                              // 식 (107) v = det(M) m₃
      var M = camM(P);
      return sc(M[2], det3(M));
    }
    function pinvP(P) {                                      // 식 (109) P† = Pᵀ(PPᵀ)⁻¹
      var Pt = mT(P);
      return mm(Pt, minv(mm(P, Pt)));
    }
    function backProjectRay(P, x) {                          // 식 (110) X(λ) = P†x + λC
      return { A: mv(pinvP(P), x), C: camCenter(P) };
    }
    function depthOf(P, X) {                                 // 식 (113)(116)
      var M = camM(P), x = mv(P, X);
      return Math.sign(det3(M))*x[2]/(nrm(M[2])*(X[3] === 0 ? 1 : X[3]));
    }
    // 식 (140)~(144) 카메라 DLT
    function dltP(X3, x2, normalize) {
      var n = X3.length, i;
      var Tn = eye(3), Un = eye(4), xs = x2.map(hnorm), Xs = X3.map(hnorm);
      if (normalize !== false) {
        var N = normalizePts(x2); Tn = N.T; xs = N.pts;
        // 3D 정규화: centroid 0, 평균거리 √3
        var c = [0,0,0];
        Xs.forEach(function (X) { for (i = 0; i < 3; i++) c[i] += X[i]; });
        c = c.map(function (v) { return v/n; });
        var d = 0;
        Xs.forEach(function (X) { d += nrm(sub(X.slice(0,3), c)); });
        d /= n;
        var s = d < 1e-300 ? 1 : Math.sqrt(3)/d;
        Un = [[s,0,0,-s*c[0]],[0,s,0,-s*c[1]],[0,0,s,-s*c[2]],[0,0,0,1]];
        Xs = Xs.map(function (X) { return mv(Un, X); });
      }
      var A = [];
      for (i = 0; i < n; i++) {
        var X = Xs[i], x = xs[i], Z = [0,0,0,0];
        A.push(Z.concat(sc(X, -x[2])).concat(sc(X, x[1])));
        A.push(sc(X, x[2]).concat(Z).concat(sc(X, -x[0])));
      }
      var p = nullvec(A);
      var Pn = [p.slice(0,4), p.slice(4,8), p.slice(8,12)];
      return mm(minv(Tn), mm(Pn, Un));                        // 식 (156)
    }

    // ---- IAC 와 소실점 (식 171~196) ----------------------------------------
    function omegaFromK(K) { var Ki = minv(K); return mm(mT(Ki), Ki); }   // 식 (177)
    function rayAngle(x1, x2, w) {                            // 식 (178)
      var a = dot(x1, mv(w, x2));
      var b = Math.sqrt(Math.max(1e-300, dot(x1, mv(w, x1))));
      var c = Math.sqrt(Math.max(1e-300, dot(x2, mv(w, x2))));
      return Math.acos(Math.max(-1, Math.min(1, a/(b*c))));
    }
    function vanishingPoint(K, d) { return mv(K, d); }        // 식 (184)
    // 식 (196) zero-skew + square pixel 가정으로 직교 소실점 셋에서 ω 를 푼다
    function omegaFromVPs(vs) {
      // w = [[w1,0,w2],[0,w1,w3],[w2,w3,w4]]
      var A = [];
      for (var i = 0; i < vs.length; i++) for (var j = i+1; j < vs.length; j++) {
        var a = hnorm(vs[i]), b = hnorm(vs[j]);
        A.push([a[0]*b[0] + a[1]*b[1], a[0]*b[2] + a[2]*b[0], a[1]*b[2] + a[2]*b[1], a[2]*b[2]]);
      }
      var w = nullvec(A);
      var W = [[w[0], 0, w[1]], [0, w[0], w[2]], [w[1], w[2], w[3]]];
      return W[0][0] < 0 ? mscale(W, -1) : W;
    }
    // ω = K⁻ᵀK⁻¹ 에서 K 복원 (Cholesky)
    function KfromOmega(w) {
      var L = chol(w);                                        // w = L Lᵀ
      if (!L) return null;
      var Ki = mT(L);                                         // K⁻¹ = Lᵀ (상삼각)
      var K = minv(Ki);
      return mscale(K, 1/K[2][2]);
    }

    // ---- F 와 E (식 202~275) ------------------------------------------------
    function fFromP(P, Pp) {                                  // 식 (212) F = e'^ P' P†
      var C = camCenter(P), ep = mv(Pp, C);
      return mm(hat(ep), mm(Pp, pinvP(P)));
    }
    function enforceRank2(F) {                                // 식 (303)
      var s = svd(F);
      var D = [[s.D[0],0,0],[0,s.D[1],0],[0,0,0]];
      return mm(s.U, mm(D, mT(s.V)));
    }
    function eightPoint(x1, x2, normalize) {                  // 식 (300)~(306)
      var N1 = (normalize === false) ? { T: eye(3), pts: x1.map(hnorm) } : normalizePts(x1);
      var N2 = (normalize === false) ? { T: eye(3), pts: x2.map(hnorm) } : normalizePts(x2);
      var A = [];
      for (var i = 0; i < N1.pts.length; i++) {
        var a = N1.pts[i], b = N2.pts[i];
        A.push([b[0]*a[0], b[0]*a[1], b[0]*a[2],
                b[1]*a[0], b[1]*a[1], b[1]*a[2],
                b[2]*a[0], b[2]*a[1], b[2]*a[2]]);
      }
      var f = nullvec(A);
      var Fn = enforceRank2([f.slice(0,3), f.slice(3,6), f.slice(6,9)]);
      var F = mm(mT(N2.T), mm(Fn, N1.T));
      return mscale(F, 1/Math.max(1e-300, E_maxabs(F)));
    }
    function E_maxabs(M) { var m = 0; M.forEach(function (r) { r.forEach(function (v) { m = Math.max(m, Math.abs(v)); }); }); return m; }
    function epipoles(F) {                                    // 식 (213) Fe = 0, e'ᵀF = 0
      return { e: hnormSafe(nullvec(F)), ep: hnormSafe(nullvecLeft(F)) };
    }
    function hnormSafe(x) { var n = nrm(x); return n < 1e-300 ? x : sc(x, 1/n); }
    function sampsonError(F, x, xp) {                         // F 품질 지표
      var Fx = mv(F, x), Ftx = mv(mT(F), xp);
      var n = dot(xp, Fx);
      return n*n/(Fx[0]*Fx[0] + Fx[1]*Fx[1] + Ftx[0]*Ftx[0] + Ftx[1]*Ftx[1]);
    }
    function eFromF(F, K, Kp) { return mm(mT(Kp || K), mm(F, K)); }        // 식 (252)
    var W_E = [[0,-1,0],[1,0,0],[0,0,1]];                     // 식 (255)
    var Z_E = [[0,1,0],[-1,0,0],[0,0,0]];
    function decomposeE(E) {                                  // 식 (262)~(274)
      var s = svd(E);
      var U = s.U, V = s.V;
      if (det3(U) < 0) U = mscale(U, -1);
      if (det3(V) < 0) V = mscale(V, -1);
      var R1 = mm(U, mm(W_E, mT(V)));
      var R2 = mm(U, mm(mT(W_E), mT(V)));
      var u3 = [U[0][2], U[1][2], U[2][2]];
      return [{ R: R1, t: u3 }, { R: R1, t: sc(u3, -1) },
              { R: R2, t: u3 }, { R: R2, t: sc(u3, -1) }];
    }
    // 네 해 중 두 카메라 앞에 점이 오는 것을 고른다 (cheirality)
    function chooseByCheirality(sols, x1, x2, K) {
      K = K || eye(3);
      var P1 = camP(K, eye(3), [0,0,0]), best = null, bestN = -1;
      sols.forEach(function (s, idx) {
        var P2 = camP(K, s.R, s.t), n = 0;
        for (var i = 0; i < x1.length; i++) {
          var X = triangulateDLT(P1, P2, x1[i], x2[i]);
          var Xh = hnorm(X);
          var d1 = mv(P1, Xh)[2], d2 = mv(P2, Xh)[2];
          if (d1 > 0 && d2 > 0) n++;
        }
        s.nFront = n;
        if (n > bestN) { bestN = n; best = idx; }
      });
      return { index: best, sols: sols };
    }

    // ---- 삼각측량 (식 316~341) ---------------------------------------------
    function triangulateDLT(P, Pp, x, xp) {                   // 식 (324)
      var a = hnorm(x), b = hnorm(xp);
      var A = [
        sub(sc(P[2], a[0]), P[0]),
        sub(sc(P[2], a[1]), P[1]),
        sub(sc(Pp[2], b[0]), Pp[0]),
        sub(sc(Pp[2], b[1]), Pp[1])
      ];
      return nullvec(A);
    }
    function reprojError(P, Pp, X, x, xp) {                   // 식 (315)
      var a = hnorm(mv(P, X)), b = hnorm(mv(Pp, X));
      var u = hnorm(x), v = hnorm(xp);
      return Math.hypot(a[0]-u[0], a[1]-u[1]) + Math.hypot(b[0]-v[0], b[1]-v[1]);
    }

    // ---- 평면 homography (식 342~384) --------------------------------------
    function hFromPlane(A, a, v) {                            // 식 (343) H = A − a vᵀ
      var H = zeros(3, 3);
      for (var i = 0; i < 3; i++) for (var j = 0; j < 3; j++) H[i][j] = A[i][j] - a[i]*v[j];
      return H;
    }
    function hFromRtn(K, Kp, R, t, n, d) {                    // 식 (351)
      var M = zeros(3, 3);
      for (var i = 0; i < 3; i++) for (var j = 0; j < 3; j++) M[i][j] = R[i][j] - t[i]*n[j]/d;
      return mm(Kp || K, mm(M, minv(K)));
    }
    function hInf(K, Kp, R) { return mm(Kp || K, mm(R, minv(K))); }        // 식 (383)
    function compatCheck(H, F) {                              // 식 (353) HᵀF + FᵀH = 0
      return madd(mm(mT(H), F), mm(mT(F), H));
    }

    // ---- Trifocal tensor (식 401~456) --------------------------------------
    // T_i = a_i b₄ᵀ − a₄ b_iᵀ  (식 407).  P = [I|0], P' = [A|a₄], P'' = [B|b₄]
    function trifocalFromP(Pp, Ppp) {
      var A = camM(Pp), a4 = camP4(Pp), B = camM(Ppp), b4 = camP4(Ppp);
      var T = [];
      for (var i = 0; i < 3; i++) {
        var ai = [A[0][i], A[1][i], A[2][i]];                 // A 의 i 번째 열
        var bi = [B[0][i], B[1][i], B[2][i]];
        var Ti = zeros(3, 3);
        for (var j = 0; j < 3; j++) for (var k = 0; k < 3; k++)
          Ti[j][k] = ai[j]*b4[k] - a4[j]*bi[k];
        T.push(Ti);
      }
      return T;
    }
    function trifocalLine(T, lp, lpp) {                       // 식 (408)(409)
      return [dot(lp, mv(T[0], lpp)), dot(lp, mv(T[1], lpp)), dot(lp, mv(T[2], lpp))];
    }
    function trifocalSum(T, x) {                              // Σ xᵢ Tᵢ
      var S = zeros(3, 3);
      for (var i = 0; i < 3; i++) for (var j = 0; j < 3; j++) for (var k = 0; k < 3; k++)
        S[j][k] += x[i]*T[i][j][k];
      return S;
    }
    // 식 (448) point transfer:  x'' = (Σ xᵢ Tᵢᵀ) l'
    function trifocalPointTransfer(T, x, lp) {
      var S = zeros(3, 3);
      for (var i = 0; i < 3; i++) for (var j = 0; j < 3; j++) for (var k = 0; k < 3; k++)
        S[k][j] += x[i]*T[i][j][k];
      return mv(S, lp);
    }
    // 식 (452) line transfer:  l'' 을 l, l' 로부터 푼다
    function trifocalLineTransfer(T, l, lp) {
      // l ∥ [ l'ᵀ Tᵢ l'' ]  →  (l^) [ … ] = 0  을 l'' 에 대한 선형계로
      var Sm = zeros(3, 3);                                    // Sm[i][k] = l'ᵀ Tᵢ (:,k)
      for (var i = 0; i < 3; i++) {
        var r = mv(mT(T[i]), lp);                              // Tᵢᵀ l'  → 길이 3
        for (var k = 0; k < 3; k++) Sm[i][k] = r[k];
      }
      var Lx = hat(l);                                         // 3×3
      return nullvec(mm(Lx, Sm));
    }
...
return { zeros: zeros, eye: eye, mm: mm, mT: mT, madd: madd, msub: msub, mscale: mscale,
             ...
             // ── Multiple View Geometry 스터디에서 더한 것 ──
             hnorm: hnorm, h2e: h2e, e2h: e2h, join: join, meet: meet,
             onLine: onLine, ptLineDist: ptLineDist,
             nullvec: nullvec, nullvecLeft: nullvecLeft, rq3: rq3, rq2: rq2, qr2: qr2,
             conicFrom5: conicFrom5, conicTangent: conicTangent,
             conicFromLines: conicFromLines, adjugate3: adjugate3, conicPoints: conicPoints,
             normalizePts: normalizePts, dltH: dltH, decomposeH: decomposeH,
             affineRectifyH: affineRectifyH, metricRectifyH: metricRectifyH,
             Kmat: Kmat, camP: camP, camM: camM, camP4: camP4,
             camCenter: camCenter, camCenterFinite: camCenterFinite, decomposeP: decomposeP,
             principalPlane: principalPlane, principalPoint: principalPoint,
             principalAxis: principalAxis, pinvP: pinvP, backProjectRay: backProjectRay,
             depthOf: depthOf, dltP: dltP,
             omegaFromK: omegaFromK, rayAngle: rayAngle, vanishingPoint: vanishingPoint,
             omegaFromVPs: omegaFromVPs, KfromOmega: KfromOmega,
             fFromP: fFromP, enforceRank2: enforceRank2, eightPoint: eightPoint,
             epipoles: epipoles, sampsonError: sampsonError,
             eFromF: eFromF, W_E: W_E, Z_E: Z_E,
             decomposeE: decomposeE, chooseByCheirality: chooseByCheirality,
             triangulateDLT: triangulateDLT, reprojError: reprojError,
             hFromPlane: hFromPlane, hFromRtn: hFromRtn, hInf: hInf, compatCheck: compatCheck,
             trifocalFromP: trifocalFromP, trifocalLine: trifocalLine, trifocalSum: trifocalSum,
             trifocalPointTransfer: trifocalPointTransfer,
             trifocalLineTransfer: trifocalLineTransfer };
  })();