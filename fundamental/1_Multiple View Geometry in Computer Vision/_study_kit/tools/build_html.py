...
def inline(text, vault):
    ...
    # 이 문서(ICP)는 색을 네 가지 쓴다. 파랑 말고 나머지 셋은 아래 마커로 적는다.
    #   {{c|...}} 진홍 #a50000 · {{r|...}} 빨강 #ff0000 · {{t|...}} 하늘 #00ffff
    # 빨강/하늘은 그림의 점 색을 가리키는 범례다 (r = source/t 시점, t = target/t+1 시점).
    for mark, cls in (("c", "hl-crimson"), ("r", "hl-src"), ("t", "hl-tgt")):
        text = re.sub(r"\{\{%s\|([^{}\n]+)\}\}" % mark,
                      r'<span class="%s">\1</span>' % cls, text)
    return text
...
:root {{
  ...
  --hl-crimson:#a50000;              /* 원문 그대로 */
  --hl-src:#ff0000;                  /* 원문 그대로 — 그림의 빨간 점(t 시점) */
  --hl-tgt:#00ffff;                  /* 원문 그대로 — 그림의 파란 점(t+1 시점) */
...
@media (prefers-color-scheme: dark) {{
  ...
  --hl-crimson:#f2736b;
  --hl-src:#ff6b6b;
  --hl-tgt:#4fd8e8;                    /* 어두운 배경에서 읽히도록 밝힌 값 */
    --hl-crimson:#f2736b;
    --hl-src:#ff6b6b;
    --hl-tgt:#4fd8e8;
...
/* ---- 원문의 나머지 세 색 (산문 쪽) ----
   수식 안의 같은 색은 \\color 매크로로 찍는다. 여기는 산문 전용이다. */
.hl-crimson {{ color:var(--hl-crimson); font-weight:640; }}
.hl-src     {{ color:var(--hl-src);     font-weight:640; }}
.hl-tgt     {{ color:var(--hl-tgt);     font-weight:640; }}
.hl-crimson mjx-container, .hl-src mjx-container, .hl-tgt mjx-container {{ color:inherit; }}

/* ---- 원문의 수식 내부 색 강조 (진홍 #a50000) ----
   이쪽은 CSS 로 못 만든다. \\color 로 찍어야 하고, 그 값은 SVG 에
   fill/stroke '속성'으로 박힌다. 속성은 CSS 프로퍼티에 지므로
   어두운 배경에서는 아래 규칙이 이겨서 읽히는 밝기로 바뀐다.
   (원문 값 자체를 바꾸는 것이 아니라 표시만 보정하는 것이다) */
@media (prefers-color-scheme: dark) {{
  :root:not([data-theme="light"]) [fill="#a50000"] {{ fill:#f2736b; }}
  :root:not([data-theme="light"]) [stroke="#a50000"] {{ stroke:#f2736b; }}
}}
:root[data-theme="dark"] [fill="#a50000"] {{ fill:#f2736b; }}
:root[data-theme="dark"] [stroke="#a50000"] {{ stroke:#f2736b; }}
@media (prefers-color-scheme: dark) {{
  :root:not([data-theme="light"]) [fill="#ff0000"] {{ fill:#ff6b6b; }}
  :root:not([data-theme="light"]) [stroke="#ff0000"] {{ stroke:#ff6b6b; }}
  :root:not([data-theme="light"]) [fill="#00ffff"] {{ fill:#4fd8e8; }}
  :root:not([data-theme="light"]) [stroke="#00ffff"] {{ stroke:#4fd8e8; }}
}}
:root[data-theme="dark"] [fill="#ff0000"] {{ fill:#ff6b6b; }}
:root[data-theme="dark"] [stroke="#ff0000"] {{ stroke:#ff6b6b; }}
:root[data-theme="dark"] [fill="#00ffff"] {{ fill:#4fd8e8; }}
:root[data-theme="dark"] [stroke="#00ffff"] {{ stroke:#4fd8e8; }}
...
window.MathJax = {{
  tex: {{
    // color·boldsymbol 은 autoload 대상이라 그냥 쓰면 네트워크를 타고,
    // 오프라인에서는 페이지의 모든 수식이 조용히 죽는다 (3_Pitfalls B6).
    // vendor/tex-svg.js 뒤에 두 컴포넌트를 이어 붙여 두었으므로
    // 여기서 패키지 목록에 더해 주기만 하면 요청 없이 등록된다.
...
// 하한 55% — 그보다 더 줄여야 하는 수식은 억지로 줄이지 않고 스크롤을 남긴다
// (읽을 수 없을 만큼 작아지는 것을 방지).
// 다른 스터디는 70%인데 이 문서만 55%로 낮췄다. 식 (37)이 15×15 F 와 15×18 G 를
// 한 줄에 늘어놓은 거대한 행렬이라 62%로도 좁은 창에서 넘쳤다.
(function () {{
  var MIN = 0.55;
  function fit() {{
    document.querySelectorAll('.mathblock').forEach(function (box) {{
      ...
      if (!eq) return;
      // 넘침 판정은 컨테이너가 아니라 박스 자신의 scrollWidth 로 한다.
      // mjx-container 의 bounding rect 는 실제로 넘치는 폭과 다를 수 있다.
      if (box.scrollWidth > box.clientWidth + 1) {{
        var avail = box.clientWidth - 2;
        var need = eq.getBoundingClientRect().width;
        var ratio = need > 0 ? Math.max(MIN, (avail / need) * 0.97) : 1;
        box.style.fontSize = (ratio * 100).toFixed(1) + '%';
        // 폰트 크기와 렌더 폭이 정확히 비례하지 않는다 (MathJax SVG 의 반올림·최소폭).
        // 한 번만 계산하면 창 폭에 따라 몇 픽셀씩 넘치는 수식이 남는다 —
        // 그래서 재보고 다시 줄이기를 몇 번 반복한다. (3_Pitfalls C7)
    ...
    // 줄바꿈이 불가능한 긴 식(예: det(2 3 4)c1 − ... = 0)이 여기 걸린다. (3_Pitfalls C9)
