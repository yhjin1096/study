...
이 스터디의 원문은 색을 두 가지로 쓴다.

  #197fb2 (파랑)  산문 강조 — 고딕/산세리프 볼드 폰트. ==강조== 로 옮긴다
  #a50000 (진홍)  수식 내부 강조 — CM 계열 수학 폰트. \\color{#a50000}{...} 로 옮긴다

진홍은 210구간이라 눈으로 찾으면 반드시 샌다. 전사할 때 이 스크립트의 출력을
수식 옆에 놓고 하나씩 대조한다.

사용법
  python3 _study_kit/tools/dump_colored.py                 # 전체
  python3 _study_kit/tools/dump_colored.py -p 7-9          # 쪽 지정
  python3 _study_kit/tools/dump_colored.py -p 7 -c red     # 색 지정 (blue|red|all)
...
def main():
    ...
    ap.add_argument("-c", "--color", default="all", choices=["all", "blue", "red"])
    ...
    wanted = {BLUE, RED} if args.color == "all" else {BLUE if args.color == "blue" else RED}
    ...
    for pno in pages:
        ...
        for i, r in enumerate(shown, 1):
            ...
            tag = f"({eq})" if eq else "  — "
            print(f"  {i:3d}. [{NAME[r['color']]:4s}] {tag:>6s}  {r['text'].strip()}")

    print("\n── 합계 ──")
    for c in (BLUE, RED):
        if c in wanted:
            print(f"  {NAME[c]:4s} #{c:06x}  {total[c]:4d}구간  {chars[c]:5d}자")
    return 0
