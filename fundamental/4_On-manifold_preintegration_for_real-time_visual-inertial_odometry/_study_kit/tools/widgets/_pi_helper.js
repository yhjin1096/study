// window.PI — 이 스터디의 위젯 12개가 공유하는 계산 라이브러리
//
//   앞부분(zeros … frame)은 0_Lie_Theory 스터디의 window.LG 를 그대로 가져온 것이다.
//   SO(3)/SE(3) 지수·로그 매핑, 오른쪽 자코비안, 3D 씬이 들어 있다.
//   뒷부분("여기부터 이 스터디에서 더한 것")이 preintegration 전용이다 —
//   IMU 시뮬레이터, 식 (34) 전방적분, 식 (37)(58)(66) preintegration,
//   식 (48) bias 1차 보정, 식 (49) residual, 수치미분, 2D 플롯.
//
//   위젯 파일에는 `//%PI%` 한 줄만 두고 tools/inject_helper.py 로 채워 넣는다.
//   고칠 일이 있으면 이 파일만 고치고 inject_helper.py 를 다시 돌린다.
//
//   검산 결과는 _study_kit/README.md 의 표를 볼 것.

if (!window.PI) window.PI = (function () {
    // ---- 일반 행렬 ------------------------------------------------------
