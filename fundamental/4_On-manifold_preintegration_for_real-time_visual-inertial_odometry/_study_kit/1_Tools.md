...
노트 .md (원본, 손으로 작성)
   ...
   +  tools/widgets/_pi_helper.js → inject_helper.py 로 위젯에 주입
   +  tools/vendor/tex-svg.js (MathJax + color + boldsymbol)
        │
        ├── python3 _study_kit/tools/inject_helper.py
        ├── python3 _study_kit/tools/build_html.py notes/노트.md
        ▼
...
> **이 스터디에는 그림이 하나도 없다.** `extract_figures.py` 와 `extract_images_bbox.py`
> 는 다른 스터디에서 쓰던 것을 그대로 두었을 뿐 여기서는 쓰지 않는다. 자세한 이유는
> `kit.conf` 의 캡션 항목과 `README.md` ② 를 볼 것.
...
이 문서는 색을 두 가지 쓴다. 파랑 `#197fb2` 는 산문 강조, 진홍 `#a50000` 은 **수식 안**이다.
합쳐 86구간이라 눈으로 찾으면 반드시 샌다. 전사 전에 뽑아 놓고 옆에 두고 쓴다.

```bash
python3 _study_kit/tools/dump_colored.py --count          # 개수만 — 전사 후 대조용
python3 _study_kit/tools/dump_colored.py -c blue          # 파랑만
python3 _study_kit/tools/dump_colored.py -p 9-11 -c red   # 9~11쪽의 진홍
```
...
위젯 11개가 `window.PI` 를 공유한다. 원본은 `tools/widgets/_pi_helper.js` 한 벌이고,
위젯 파일에는 `//%PI%` 표시만 둔다.
