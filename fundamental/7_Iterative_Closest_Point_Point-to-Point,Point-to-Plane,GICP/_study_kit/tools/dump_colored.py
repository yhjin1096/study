...
이 스터디의 원문은 색을 한 가지만 쓴다.

  #197fb2 (파랑)  산문 강조 — 고딕/산세리프 볼드 폰트. ==강조== 로 옮긴다

수식 안에는 색이 없다(검사 결과 다른 색의 span 이 0개). 따라서 \\color 는 쓰지 않는다.
...
#   이 문서(Notes on ICP)는 네 가지를 쓴다 — 지금까지 중 가장 많다.
BLUE    = 0x197FB2   # 산문 강조 (nanummjb*/SFBX1000)
CRIMSON = 0xA50000   # 수식 강조(p4·p5·p19) + 산문 강조(p12·p17·p19·p23)
RED     = 0xFF0000   # "빨간색" 범례 · sourcePoints · 수식의 source 항 (p2·p3)
CYAN    = 0x00FFFF   # "파란색" 범례 · targetPoints · 수식의 target 항 (p2·p3)
NAME = {BLUE: "blue", CRIMSON: "crimson", RED: "red", CYAN: "cyan"}
ALL = tuple(NAME)
# 빨강/하늘은 산문(범례)과 수식(항 구분)에 모두 쓰인다. 폰트로는 갈리지 않으므로
# 두 그룹에 다 넣고, 실제 분류는 덤프를 눈으로 보고 판단한다.
GROUP = {"prose": (BLUE, CRIMSON, RED, CYAN), "math": (CRIMSON, RED, CYAN)}
