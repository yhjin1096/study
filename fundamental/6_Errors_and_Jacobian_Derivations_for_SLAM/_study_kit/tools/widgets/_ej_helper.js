// window.EJ — 이 스터디의 위젯 12개가 공유하는 계산 라이브러리
//
//   앞부분은 5_VINS-Mono 스터디의 window.VM 을 그대로 가져온 것이다
//   (SO(3)/SE(3) 지수·로그, 쿼터니언과 Ω_L/Ω_R, IMU preintegration, 3D 씬,
//    난수, 수치미분, 2D 플롯, Schur complement).
//   뒷부분("여기부터 …")이 이 문서 전용이다 —
//   핀홀 투영 π_h/π_k 와 렌즈 왜곡, 합성 이미지와 해석적 ∇I,
//   Plücker 직선·orthonormal (U,W) 표현, GN/LM 한 스텝.
//
//   이 문서는 자코비안 문서이므로 위젯 대부분이
//   "해석적 자코비안 vs numJac(수치미분)" 대조형이다.
//
//   위젯 파일에는 `//%EJ%` 한 줄만 두고 tools/inject_helper.py 로 채워 넣는다.
