...
원본 PDF (ref/)
        ...
        ├── python3 _study_kit/tools/extract_images_bbox.py   ← 그림 9개 (캡션 없음)
        ▼
노트 .md (원본, 손으로 작성)
   +  tools/widgets/*.html (Canvas 위젯 12개)
   +  images/*.png (PDF 에서 뽑은 그림 9개)
   +  tools/widgets/_ej_helper.js → inject_helper.py 로 위젯에 주입
...
> **이 스터디는 그림 9개에 캡션이 하나도 없다** — 전문에서 `Figure`/`Fig.`/`Table` 이 0회다.
> 그래서 `extract_images_bbox.py` 만 쓴다. `extract_figures.py`(캡션 기반)는 쓸 수 없다.
> 파일명은 캡션 대신 내용을 보고 지었다. 자세한 것은 `kit.conf` 의 캡션 항목과 `README.md` ② 를 볼 것.
...
위젯 12개가 `window.EJ` 을 공유한다. 원본은 `tools/widgets/_ej_helper.js` 한 벌이고,
위젯 파일에는 `//%VM%` 표시만 둔다.
