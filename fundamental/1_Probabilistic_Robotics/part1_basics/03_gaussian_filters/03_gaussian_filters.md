...
> **더 자세히**: $z_t$와 $C_t\bar\mu_t$가 각각 어디서 오는 값인지, 둘의 시점 관계, 그리고 LiDAR를
> 예로 한 구체적 계산은 **[부록 B](#부록-b-innovation-읽기-실제-측정과-예상-측정의-차이)** 에 정리해두었다.
...
**$L_t(x_t)$가 정말로 $x_{t-1}$에 의존하지 않는지 빠르게 확인해보자.** 이를 위해
$\Psi_t = (A_t^T R_t^{-1}A_t + \Sigma_{t-1}^{-1})^{-1}$을 다시 대입하고 위의 항들을 전개한다. (책은 $x_{t-1}$을 포함한 항에 밑줄을,
...
즉 **새로운 수학은 없다.** 3.2.4절에서 $A_t x_{t-1} + B_t u_t$였던 자리에 $g(u_t,\mu_{t-1}) + G_t(x_{t-1}-\mu_{t-1})$을,
$C_tx_t$였던 자리에 $h(\bar\mu_t)+H_t(x_t-\bar\mu_t)$를 넣고 그대로 따라가면 된다.
...
>
> **가중치 합 확인**:
> $\dfrac{\lambda}{n+\lambda} + 2n \cdot \dfrac{1}{2(n+\lambda)} = \dfrac{\lambda+n}{n+\lambda} = 1$ ✔ —
> $w_m$의 합은 정확히 1이다.
...
### A.4.4 곁가지 — 식 (4) 자체에는 선형성이 필요하지 않다
...
# 부록 B — innovation 읽기: 실제 측정과 예상 측정의 차이

Table 3.1 라인 5의 괄호 안 $z_t - C_t\bar\mu_t$를 정확히 읽기 위한 부록이다. 두 값이 각각 어디서 오는지,
둘의 시점 관계, 그리고 LiDAR를 예로 한 구체적 계산을 다룬다.

$$\mu_t = \bar\mu_t + K_t\,\underbrace{(z_t - C_t\,\bar\mu_t)}_{\text{innovation}}$$
...
|---|---|---|
| **무엇인가** | 실제 측정값 | 예상 측정값 |
| **어디서 오나** | 센서 하드웨어 | 계산 (행렬 곱) |
| **무엇에서 나오나** | **참 상태** $x_t$ | **예측 평균** $\bar\mu_t$ |
| **로봇이 아는가** | 예 — 숫자로 들어온다 | 예 — 자기가 계산했다 |
| **근거 식** | $z_t = C_t x_t + \delta_t$ (식 (5)) | $C_t$를 $\bar\mu_t$에 적용 |
| **성격** | 사실 | "내 추정이 맞다면 봤어야 할 값" |

$C_t$는 **상태를 센서 값으로 번역하는 표**다. 그 표를 참 상태에 적용하면 실제 측정이 나오고(자연이 한다),
예측 평균에 적용하면 예상 측정이 나온다(로봇이 한다).

## B.2 시점 — 둘 다 같은 시각의 값이다

가장 헷갈리는 지점이다. **시간 차이가 아니라 출처 차이다.**

```
      시각 t−1                        시각 t
   ────────────────────────────────────────────────────────────────────
   bel(x_{t-1}) ─── u_t 반영 ───▶   mu_bar_t , Sigma_bar_t     로봇의 추정 (앎)
   (mu_{t-1}, Sigma_{t-1})                  │
                                            │  C_t 를 곱한다
                                            ▼
                                       C_t mu_bar_t            예상 측정값 (계산으로 만든 값)
...
| | 같은 것 | 다른 것 |
|---|---|---|
| | 시각 — 둘 다 $t$ 시점의 이야기 | 출처 — 하나는 추정에서 계산, 하나는 실제 세계에서 관측 |

즉 **"내 생각 vs 현실"의 차이**이며, $\bar\mu_t$와 $x_t$는 둘 다 시각 $t$의 것이다. 하나는 로봇의 추측,
하나는 현실이다.
...
직선 레일 위의 로봇. 뒤쪽 원점(0m)에 벽이 있고, LiDAR가 뒤를 향해 벽까지의 거리를 잰다.

$$x = \begin{pmatrix} p \\ v \end{pmatrix} \;\;(\text{위치, 속도}), \qquad z = (\text{벽까지 거리}) = p$$

$C_t$는 "상태에서 위치만 뽑아낸다"는 표가 된다.

$$C_t = \begin{bmatrix} 1 & 0 \end{bmatrix} \quad (1\times 2), \qquad C_t x = \begin{bmatrix} 1 & 0 \end{bmatrix}\begin{pmatrix} p \\ v \end{pmatrix} = p$$

> **$C_t$가 존재하는 이유가 여기 보인다** — 센서는 상태의 **일부만** 본다. LiDAR는 위치는 알려주지만
> 속도는 직접 알려주지 않는다.
...
| 단계 | 계산 | 의미 |
|---|---|---|
| ① prediction 결과 | $\bar\mu_t = (5.0,\; 1.0)^T$ | "위치 5.0m, 속도 1.0m/s 일 것이다" |
| ② 예상 측정값 | $C_t\bar\mu_t = 5.0\,\text{m}$ | "내 예측이 맞다면 LiDAR는 5.0m를 읽을 것" |
| ③ 실제 LiDAR 스캔 | $z_t = 5.3\,\text{m}$ | 실제로 들어온 값 |
| ④ innovation | $z_t - C_t\bar\mu_t = +0.3\,\text{m}$ | "예상보다 0.3m 더 멀리 있다" |
...
$K_t = (0.6,\; 0.2)^T$ ($2\times 1$)라고 하면:

$$\mu_t = \begin{pmatrix} 5.0 \\ 1.0 \end{pmatrix} + \begin{pmatrix} 0.6 \\ 0.2 \end{pmatrix} \times 0.3 = \begin{pmatrix} 5.18 \\ 1.06 \end{pmatrix}$$

**LiDAR는 위치만 측정했는데 속도도 $1.0 \to 1.06$으로 보정되었다.** "예상보다 더 멀리 왔다"는 사실은
"위치가 조금 앞이다"로도, "속도가 조금 빠르다"로도 설명될 수 있기 때문이다. $K_t$가 그 배분을 결정하며,
배분 비율은 $\bar\Sigma_t$에 담긴 **위치-속도 상관**에서 나온다.

$$K_t = \underbrace{\bar\Sigma_t C_t^T}_{\text{상관 정보}} (C_t \bar\Sigma_t C_t^T + Q_t)^{-1}$$

이것이 KF가 "직접 관측할 수 없는 상태 변수도 추정한다"는 말의 실제 메커니즘이다.

## B.4 innovation이 말해주는 것

| innovation | 의미 | 결과 |
|---|---|---|
| $= 0$ | 예측이 정확했다 | 보정 없음. $\mu_t = \bar\mu_t$ |
| $> 0$ | 예상보다 멀다 | 보정 방향 $+$ |
| $< 0$ | 예상보다 가깝다 | 보정 방향 $-$ |
| 매우 큼 | 예측이 틀렸거나 측정이 이상치 | **gating** — 임계값을 두고 그 측정을 버린다 |

마지막 행이 실무에서 중요하다. innovation의 크기는 $C_t\bar\Sigma_t C_t^T + Q_t$로 정규화해서
**Mahalanobis 거리**로 판정한다. 이는 2장에서 본 evidence $p(z_t)$가 작다는 것의 구체적 형태이며,
동적 장애물이나 반사로 인한 이상치를 거부하는 데 쓰인다.

## B.5 왜 "innovation(새 정보)"이라 부르는가

| 항 | 담고 있는 것 |
|---|---|
| $C_t\bar\mu_t$ | 이미 알던 것으로부터 계산 가능한 부분 → **새롭지 않다** |
| $z_t - C_t\bar\mu_t$ | 예측으로는 설명되지 않는 부분 → **이것만이 새 정보** |

측정값 $z_t$ 전체가 새 정보가 아니다. **예측이 이미 맞힌 부분은 새로울 것이 없고, 어긋난 부분만
belief를 바꿀 자격이 있다.** 그래서 라인 5가 $z_t$ 자체를 쓰지 않고 차이를 쓴다.

$$\mu_t = \underbrace{\bar\mu_t}_{\text{이미 알던 것}} + K_t \cdot \underbrace{(z_t - C_t\bar\mu_t)}_{\text{놀란 만큼}}$$
...
```
   상태 공간 (n차원)                        측정 공간 (k차원)
   ─────────────────────────────────────────────────────────────
        mu_bar_t  ──── C_t ────▶            C_t mu_bar_t    예상
                                            z_t             실제
                                        ────────────────────
                                        z_t − C_t mu_bar_t  innovation (k차원)
        mu_t      ◀──── K_t ────
```

| 행렬 | 방향 | 뜻 | 크기 |
|---|---|---|---|
| $C_t$ | 상태 → 측정 | "이 상태라면 센서가 무엇을 볼까" | $k \times n$ |
| $K_t$ | 측정 → 상태 | "이만큼 어긋났으면 상태를 얼마나 고칠까" | $n \times k$ |

$C_t$와 $K_t$는 **서로 반대 방향의 번역기**다. B.3 예제에서 innovation은 스칼라 하나(거리 1개)였지만
보정은 2차원 상태에 일어났다 — $K_t$가 $2\times 1$이라서 가능한 일이다.

## B.7 실제 LiDAR는 비선형이다 → 7장 EKF

B.3의 예제는 1차원 레일이라 $C_t$가 행렬로 떨어졌다. 실제 2D LiDAR와 랜드마크는 다르다.
로봇 pose $x = (x, y, \theta)^T$, 지도에서 아는 랜드마크 위치 $(m_x, m_y)$에 대해:

$$\text{예상 측정값} = \begin{pmatrix} \sqrt{(m_x - x)^2 + (m_y - y)^2} \\[2pt] \mathrm{atan2}(m_y - y,\; m_x - x) - \theta \end{pmatrix} \quad \begin{matrix}\leftarrow \text{거리} \\ \leftarrow \text{방향}\end{matrix}$$

$\sqrt{\cdot}$와 $\mathrm{atan2}$ 때문에 **행렬 곱으로 쓸 수 없다.** 그래서 $C_t\bar\mu_t$를 비선형 함수로 바꾼다.

| 필터 | innovation | 비고 |
|---|---|---|
| **KF** | $z_t - C_t\bar\mu_t$ | 선형. 행렬 곱 |
| **EKF** (3.3절) | $z_t - h(\bar\mu_t)$ | $h$는 비선형 측정함수. $C_t$ 자리에 Jacobian $H_t$가 들어간다 |

실제 LiDAR로 하는 일 (7장 EKF Localization):

1. 스캔에서 랜드마크를 뽑는다 → $z_t = (\text{실측 거리},\; \text{실측 방향})$
2. 예측 pose $\bar\mu_t$와 지도로 계산한다 → $h(\bar\mu_t) = (\text{예상 거리},\; \text{예상 방향})$
3. 둘의 차 → innovation
4. $K_t$로 pose를 보정한다

**innovation의 개념은 완전히 동일하고, "예상 측정값을 어떻게 계산하는가"만 바뀐다.** 선형이면 행렬 곱,
비선형이면 함수 호출이다.
...
| | |
|---|---|
| $z_t$ | 참 상태 $x_t$에서 실제로 발생한 측정. 센서가 준 값 |
| $C_t\bar\mu_t$ | 예측 상태 $\bar\mu_t$에서 발생할 것으로 계산한 측정. 로봇이 만든 값 |
| **시점** | 둘 다 시각 $t$. 시간 차이가 아니다 |
| **차이의 정체** | "내 생각 vs 현실"의 불일치 $=$ innovation $=$ 유일한 새 정보 |

> **한 문장**: $C_t\bar\mu_t$는 "내 추정이 맞다면 봤어야 할 것", $z_t$는 "실제로 본 것"이고,
> Kalman filter는 그 둘이 어긋난 만큼만 상태를 고친다.
