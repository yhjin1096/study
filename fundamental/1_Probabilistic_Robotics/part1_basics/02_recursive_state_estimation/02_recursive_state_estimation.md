...
**라인 3 — Prediction (control update)** — 유도는 아래 2.4.3절 식 (33)
...
**라인 4 — Measurement update (correction)** — 유도는 아래 2.4.3절 식 (30)
...
>
> 책의 추가 언급: **독자는 $p(x_t \mid x_{t-1}, u_t) = p(x_t \mid x_{t-1})$임을 빠르게 납득해야 한다.**
> (제어가 무작위로 선택된다는 가정 하에서, $x_{t-1}$이 주어졌을 때 $u_t$ 자체는 $x_{t-1}$에 대한 정보를
> 주지 않으므로 — 다만 $x_t$를 예측하는 데는 $u_t$가 필요하므로 표기에는 남겨둔다.)

**(37) 마지막 정리 — 라인 3이 유도됨** — 책 (2.41)
