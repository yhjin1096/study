...
이유 — "그래프"는 차트와 헷갈린다).~~
→ **이 판단은 18장에서 뒤집혔다. `그래프` 는 한국어를 쓴다** — 아래 "18장에서 추가된 용어" 참조.
...
| 쓸 것 | 쓰지 말 것 |
|---|---|
| **BFS** / breadthfirst search | 너비 우선 탐색 (**약어·원어 그대로**) |
| **frontier** | 경계면 / 전선 / 프론티어 |
| vertex-centric / edge-centric | 정점 중심 / 간선 중심 |
| **push (top-down)** / **pull (bottom-up)** | 밀기 / 당기기, 하향식 / 상향식 (16장 규약 그대로) |
| direction-optimized | 방향 최적화 |
| **small world graph** | 좁은 세상 그래프 (**합성 고유명사 — 통째로 영어**) |
| **compare-and-swap** / `compare_exchange_strong` | 비교 후 교환 |
| `atomic_ref` / `thread_scope_block` / `thread_scope_device` | (식별자 그대로) |
| `memory_order_relaxed` / memory order | 메모리 순서 (**서술은 "메모리 순서" 허용, 인자명은 식별자**) |
| **cooperative groups** / `grid.sync()` / `grid_group` | 협력 그룹 (16장 규약 그대로) |
| `cudaLaunchCooperativeKernel` / `cudaOccupancyMaxActiveBlocksPerMultiprocessor` | (API 이름 그대로) |
| **GraphBLAS** / linear algebraic formulation | (고유명사·개념 이름 그대로) |
| maze routing / net terminal / blockage | 미로 배선 / 넷 단자 / 막힘 (**아래 참조**) |
| **degree** (정점의) | 차수 (**아래 참조**) |
| bucket | 버킷 |
| idempotent | 멱등 — **"멱등"은 한국어를 쓴다** (아래 참조) |

**18장에서 규칙을 고쳤다 — `그래프` 는 한국어를 쓴다 (16장 규약 뒤집음)**

16장 규칙표에 "`graph` 자체는 영어다 (\"그래프\"는 차트와 헷갈린다)"라고 적어 두었는데,
**18장을 쓰면서 이 판단을 뒤집는다.** 근거 셋:

1. **이 스터디의 판단 기준을 그대로 적용하면 한국어다.** "한국어 교재에서 그 번역어가
   실제로 통용되는가?" — 그래프 이론·그래프 알고리즘·방향 그래프·인접 행렬은
   한국어 알고리즘 교재의 **완전한 표준**이다. `sparse matrix` 를 영어로 둔 것과 정반대 상황이다.
2. **`정점`·`간선` 을 한국어로 정해 놓고 `graph` 만 영어로 두면 잡종이 된다.**
   "graph 의 정점" 보다 "그래프의 정점" 이 읽힌다.
3. **차트와 헷갈릴 여지가 없다.** 18장에서 `그래프` 가 나오는 모든 자리에
   정점·간선·인접 행렬이 함께 있다. 8장의 `grid`(격자 vs CUDA grid)와 달리
   **같은 노트 안에 경쟁하는 뜻이 없다.**

| 쓸 것 | 쓰지 말 것 |
|---|---|
| **그래프**, 방향 그래프, 무향 그래프, 인접 행렬 | graph, 방향 graph |
| `graph traversal`, `small world graph`, `GraphBLAS` | **영어** — 장 제목·합성 고유명사·API 이름 |

→ 16장 노트의 `그래프` 15건은 **이 규칙에 이미 부합**한다. 되돌릴 것 없음.

**18장에서 정한 경계 — 도메인 용어 셋**

| 쓸 것 | 왜 |
|---|---|
| **degree**(영어) | "정점의 degree", "평균 degree", "degree 분포". 한국어 "차수"는 다항식 차수와 겹치고, 이 장이 반복해서 쓰는 **고유한 개념 이름**이다 |
| **maze routing**(영어), **net terminal**(영어), **배선 블록**(한국어) | 앞의 둘은 EDA 분야의 고유 용어이고 한국어 음차가 흔들린다. `wiring block` 은 평범한 합성어라 한국어 |
| **멱등**(한국어) | 한국어 수학·CS 교재의 표준(멱등원·멱등법칙). 다만 형용사 원문을 병기할 때는 `멱등(idempotent)` |

**18장에서 추가로 한국어를 쓰는 것**: **정점**·**간선**(16장 규약), **출발**·**도착**,
**이웃**, **최단 경로**, **지름**(diameter), **역추적**, **경쟁**(contention, 9장 규약),
**부하 균형**·**부하 불균형**, **작업 복잡도**는 한국어가 표준이다.
반면 **`work efficiency` 는 영어**다 (전역 규칙표에 이미 있다) —
"work efficient 하지 않다" 처럼 **영어 형용사 + 한국어 서술**로 쓴다.

**`배` / `×` 경계 — 18장에서 나온 사례.**
- "**$10^7\times$**", "**9 → 2**" → **두 값의 비교 → `×`** 또는 화살표
- 18장 본문에도 순수한 `배` 는 없었다. **`배선`(wiring)이 grep 에 걸리니 눈으로 거른다** —
  17장의 `두 배열`·`배정`과 같은 종류의 오탐이다.

**18장에서 되짚은 앞 장 규약** (전부 그대로 지켰다)

| 용어 | 어디서 정했나 |
|---|---|
| `latency` (지연시간 ✗) | 전역 규칙표 |
| `privatization` (사유화 ✗) | 전역 규칙표 · 9장 |
| `atomic operation` (원자적 연산 ✗) | 전역 규칙표 |
| `synchronization` 명사는 영어 / **동기화하다**는 한국어 | 전역 규칙표 |
| `control divergence` (제어 분기 ✗) | 전역 규칙표 |
| `coalescing` / `coalesced` (병합 ✗) | 전역 규칙표 |
| `occupancy` (점유율 ✗) | 전역 규칙표 |
...
| 쓸 것 | 쓰지 말 것 |
|---|---|
| **MPI** / **NCCL** / **NVSHMEM** / **CUDA-aware MPI** | (규격·라이브러리 이름 그대로) |
| **rank** | 순위 / 랭크 |
| **communicator** | 통신자 |
| **stencil** / **halo** / **halo exchange** | 스텐실 / 헤일로 / 경계 교환 |
| **stream** / **event** / **stream priority** | 스트림 / 이벤트 |
| **pinned memory** / **DMA** / **paging** | 고정 메모리 / 페이징 |
| **barrier** | 장벽 |
| **collective** | 집단 연산 |
| **interconnect** / **network** | 상호연결(망) / 통신망 / 네트워크 |
| **one-sided** / **put** / **get** / **PE** / **symmetric heap** | 단방향 / 넣기 / 가져오기 |
| **프로세스** / **노드** / **메시지** / **클러스터** | process / node / message |
| **grid** / **grid point** (8장 규약 그대로) | 격자 / 격자점 |

**① `프로세스`·`노드`·`메시지`는 한국어다 — `thread` 가 영어인 것과 모순이 아니다.**
`thread` 는 `threadIdx`·`thread block`·`thread coarsening` 처럼
**CUDA 실행 모델의 식별자와 한 몸**이라 영어로 둔다.
반면 `프로세스`는 이 책 어디에서도 식별자로 나오지 않는 OS 개념이고,
`노드`는 16·18장에서 그래프의 꼭짓점을 **정점**으로 못박아 두었기 때문에
"클러스터의 노드" 와 충돌하지 않는다.
`메시지`도 마찬가지 — 다만 `Message Passing Interface` 를 풀어 쓸 때는 영어 그대로다.

**② `interconnect`·`network` 는 영어로 통일한다.**
초안에서 같은 문단 안에 **상호연결 / 통신망 / 네트워크** 셋이 섞여 나왔다.
셋 다 원문의 `interconnect` 와 `network` 를 옮긴 것인데,
`bandwidth`·`latency` 를 영어로 두는 전역 규칙과 같은 층위의 하드웨어 용어이므로
**영어 하나로 못박는다**. 15장의 `sorting network` 를 영어로 둔 것과 같은 이유다.

**③ `배치` 는 여전히 "arrangement" 다** (19장 규약).
23장에서 나오는 `배치` 다섯 번은 전부 `row-major 배치`·`partition 의 배치`처럼
**놓인 모양**을 가리킨다. 묶음이라는 뜻의 `batch` 는 23장에 나오지 않는다.

**④ `축약` 은 동작, `reduction` 은 패턴 이름** (22장 규약).
`MPI_Allreduce` 의 `op` 를 설명할 때는 **축약 연산**,
"L2 norm 을 구하는 `reduction`" 처럼 패턴을 가리킬 때는 영어다.

**⑤ `블록` 이라는 한국어는 쓰지 않는다.**
`block` 은 thread block 을 가리키는 예약어다.
초안의 "`if` 블록", "연속 블록" 은 각각 **"`if` 문"**, **"연속 구간"** 으로 고쳤다.

**`배` / `×` 경계 — 23장에서 나온 사례.**
- "$2.0\times$"(1D 대 2D 분할의 통신량 비), "$1.45\times$"(겹치기 이득),
  "$64\times$ 늘지만"(계산과 통신이 같이 커지는 비교)
  → **전부 두 값의 비교 → `×`**
- 23장 본문에도 순수한 `배` 는 없었다.
...
| 쓸 것 | 쓰지 말 것 |
|---|---|
| **G80 / B200 / NVLink / PCIe / HBM** | (제품·규격 이름 그대로) |
| **hyperscaler** | 하이퍼스케일러 |
| **computational thinking** | 계산적 사고 (1장 규약 그대로) |
| **roofline** / **임계값**(ridge point) | 루프라인 (**임계값은 한국어** — 5장 규약) |
| **lithography mask** / feature size | 노광 마스크 / 공정 미세화 (**첫 등장 병기 허용**) |

**24장은 새 규칙이 필요 없었다.** 서술 장이라 앞 장들의 규약만으로 충분했다.
다만 두 가지를 확인해 둔다.

**① `bandwidth` 는 언제나 영어.** 24.2절이 `memory bandwidth`·`PCIe bandwidth`·
`NVLink bandwidth` 를 나란히 쓰는데 **셋 다 영어**다 (전역 규칙표).

**② 서술 장에서도 `배`/`×` 규칙은 같다.**
"$6{,}377\times$ 늘었다"·"$69\times$ 올랐다"는 **두 값의 비교**이므로 `×` 다.
24장 본문에도 순수한 `배` 는 없었다.
...
| 쓸 것 | 쓰지 말 것 |
|---|---|
| **Unified Memory** / **UVAS** / **managed memory** | 통합 메모리 / 관리 메모리 (**고유 이름은 영어**) |
| **page** / **page fault** / **paging** | 페이지 / 페이지 폴트 (23장 규약 연장) |
| **migration** / **mapping** / **coherence** / **invalidate** | 이주 / 매핑 / 일관성 / 무효화 |
| **zero-copy** / **pinned** / **interconnect** | 제로카피 / 고정 / 상호연결(망) (23장 규약) |
| **device** / **host** | 장치 / 호스트 — 단, **`I/O 장치` 는 한국어** (아래 ① 참조) |
| **주소 공간** / **가상 주소** / **물리 주소** / **포인터** / **역참조** | address space / virtual address / pointer (**아래 ② 참조**) |
| **통합 주소 공간**(서술) vs **Unified Memory**(이름) | 둘을 섞어 쓰지 않는다 |
| **비트**(bit 수를 셀 때) / **바이트** | bit / byte |

**① `device` 는 영어, `I/O 장치` 는 한국어.**
부록 C 는 두 종류의 "device" 를 한 문단에서 쓴다 —
CUDA 의 **device**(GPU)와 **I/O device**(디스크 컨트롤러·NIC)다.
앞의 것은 `host`/`device` 짝으로 책 전체에서 영어였으므로 그대로 두고,
뒤의 것은 CUDA 개념이 아니므로 **`I/O 장치`** 로 적었다.
`device` 를 `장치` 로 옮기면 두 개념이 한 낱말로 뭉개진다.
초안에서 "이 초기 장치들"·"GPU 장치는" 처럼 CUDA device 를 한국어로 쓴 곳 4군데를
전부 `device` 로 되돌렸다.

**② 주소 공간 용어는 한국어다 — `memory` 가 영어인 것과 모순이 아니다.**
`global memory`·`shared memory` 는 **CUDA 의 메모리 종류 이름**이라 영어로 둔다.
반면 `가상 주소`·`물리 주소`·`주소 공간`·`포인터`·`역참조` 는
**CUDA 가 아니라 컴퓨터 구조 일반의 개념**이고 한국어가 표준 표기다.
경계는 "CUDA 문서에 그 낱말이 API 이름·수식어로 박혀 있는가" 하나로 갈랐다.
그래서 `Unified Virtual Address Space` 는 **이름이라 영어**,
"host 와 device 가 가상 주소 공간을 공유한다"는 **서술이라 한국어**다.

**③ `page` 는 영어, `쪽` 은 책의 쪽번호 전용.**
이 부록에서 `page` 는 메모리 page 를 뜻하는데, 이 스터디는 `쪽` 을
**책 쪽번호**(`책 p.625`, `PDF 647쪽`)에만 쓴다.
둘을 같은 낱말로 적으면 "page fault 가 난 쪽" 같은 문장이 나온다.
부록 C 노트에서 `쪽` 은 전부 책 쪽번호이거나 `CPU 쪽`/`양쪽` 이고,
메모리 page 는 **한 번도 한국어로 쓰지 않았다.**

**④ `블록` 금지는 23장 규약 그대로.**
`block` 은 thread block 예약어다. 부록 C 노트의 `블록` 은 0개다.

**⑤ 6장 checklist 항목은 원문 이름으로 부른다.**
초안에서 "6장 checklist 의 ② 메모리 병합" 이라고 썼는데,
6.8절의 실제 항목 이름은 **`coalesced global memory 접근`**(memory utilization ①)이다.
`coalescing` 은 영어이고(20장 규약), 번호도 틀렸다.
**checklist 를 인용할 때는 6장 노트의 표에서 이름과 번호를 그대로 가져온다.**

**`배` / `×` 경계 — 부록 C 에서 나온 사례.**
- "$314\times$ 벌어진다"(손익분기가 $T_f$ 에 따라 벌어지는 폭),
  "$12\times$ 나빠졌다"(interconnect 비율의 18년 변화),
  "$59\times$ 더 멀어진다"(PCIe 대 NVLink) → **전부 두 값의 비교 → `×`**
- 부록 C 본문에도 순수한 `배` 는 없다. 다만 **"정확히 두 배다"**($2^{49} = 2 \times 2^{48}$)는
  비교가 아니라 **배수 그 자체**를 말하는 서술이므로 한국어 `배` 가 맞다 —
  21장에서 "부피가 10배면 일은 100배" 를 한국어로 쓴 것과 같은 자리다.
