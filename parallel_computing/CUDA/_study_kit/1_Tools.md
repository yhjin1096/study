...
heading_size  = 11.5        # 절 제목 13.4pt · 머리글 11.0pt 사이
heading_gap   = 8           # 두 줄로 넘어가는 절 제목을 잇는다
brand         = PMPP 5e
...
```bash
# 이 책에서 자동 추출이 안 되는 유일한 항목 (책 전체에서 Table 은 이것 하나뿐이고,
# 캡션이 표 위에 있어 "캡션 위쪽을 자르는" 기본 방식이 실패한다)
