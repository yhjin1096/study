...
#include <local_ba_g2o_with_wheel/local_ba_g2o/random.hpp>
#include <local_ba_g2o_with_wheel/local_ba_g2o/frame.hpp>

#include <local_ba_g2o_with_wheel/optimize/optimizer.hpp>

#include <local_ba_g2o_with_wheel/data_io/dataio.hpp>
...
void publishFramePose(const Mat44_t& pose, tf2_ros::TransformBroadcaster& broadcaster, const std::string& frame_name, const int& frame_id) {
    geometry_msgs::TransformStamped transform;
    transform.header.stamp = ros::Time::now();
    transform.header.frame_id = "map";
    transform.child_frame_id = frame_name + "_" + std::to_string(frame_id);
...
void publishOptimalPoses(const Optimizer& optimizer, tf2_ros::TransformBroadcaster& broadcaster, int num_frames)
{
...
void setObservations(std::vector<Frame>& frames, const std::vector<Landmark>& gt_landmarks)
{
    ...
    for(auto& frame : frames)
    {
        ...
        for(const auto& landmark : landmarks)
        {
            ...
            if(vertex_it != optimizer.optimizer_.vertices().end())
            {
                ...
                if(vtx)
                {
                    auto vertex_it_noise = optimizer.optimizer_.vertices().find(frame.id_);
                    if(vertex_it_noise == optimizer.optimizer_.vertices().end())
                        continue;

                    g2o::VertexSE3Expmap* vtx_noise = dynamic_cast<g2o::VertexSE3Expmap*>(vertex_it_noise->second);
                    if(vtx_noise)
                    {
                        Mat44_t noise_pose = Mat44_t::Identity();
                        noise_pose.block<3,3>(0,0) = vtx_noise->estimate().rotation().toRotationMatrix();
                        noise_pose.block<3,1>(0,3) = vtx_noise->estimate().translation();

                        // 3D -> 2D 투영
                        Eigen::Vector4d point_world(vtx->estimate()(0), vtx->estimate()(1), vtx->estimate()(2), 1.0);
                        Eigen::Vector4d point_frame = noise_pose * point_world;
...
void setNoiseLandmarks(std::vector<Landmark>& noise_landmarks, const std::vector<Frame>& frames, const std::vector<Landmark>& gt_landmarks)
{
    noise_landmarks.clear();
    noise_landmarks.reserve(gt_landmarks.size());

    for(const auto& landmark : gt_landmarks)
    {
        std::vector<std::pair<int, Vec2_t>> observations;  // frame_id와 observation pair
        for(const auto& frame : frames)
        {
            if(frame.observations_.find(landmark.id_) != frame.observations_.end())
            {
                observations.push_back({frame.id_, frame.observations_.at(landmark.id_)});
            }
        }

        if(observations.size() < 2) continue;

        // triangulation을 위한 행렬 구성
        Eigen::MatrixXd A(2 * observations.size(), 4);
        for(size_t i = 0; i < observations.size(); ++i)
        {
            const auto& frame = frames[observations[i].first];
            const auto& observation = observations[i].second;

            // 카메라 중심에서의 normalized ray 계산
            double x = (observation(0) - frame.intrinsic_(0,2)) / frame.intrinsic_(0,0);
            double y = (observation(1) - frame.intrinsic_(1,2)) / frame.intrinsic_(1,1);
            Vec3_t ray(x, y, 1.0);
            ray.normalize();

            // 카메라 좌표계에서의 ray를 월드 좌표계로 변환
            Vec3_t ray_world = frame.pose_w2c_noise_.block<3,3>(0,0) * ray;
            Vec3_t camera_center = frame.pose_w2c_noise_.block<3,1>(0,3);

            // DLT (Direct Linear Transformation) 행렬 구성
            Vec3_t ray_cross_camera = ray_world.cross(camera_center);
            A.row(2*i) = Vec4_t(ray_cross_camera(0), ray_cross_camera(1), ray_cross_camera(2), 0.0);
            A.row(2*i+1) = Vec4_t(ray_world(0), ray_world(1), ray_world(2), -camera_center.dot(ray_world));
        }

        // SVD를 통한 triangulation
        Eigen::JacobiSVD<Eigen::MatrixXd> svd(A, Eigen::ComputeFullV);
        Vec4_t X = svd.matrixV().col(3);
        Vec3_t point_3d = X.head<3>() / X(3);

        // 노이즈가 있는 landmark 생성
        noise_landmarks.push_back(Landmark(point_3d, landmark.id_));
    }

    std::cout << "Generated " << noise_landmarks.size() << " noise landmarks from " 
              << gt_landmarks.size() << " GT landmarks" << std::endl;
}
...
int main(int argc, char **argv)
{
    ...
    while(ros::ok())
    {
        ...
        if(!use_data)
        {
            ...
            //frames
            for(int i = 0; i < num_frames; ++i)
                frames.push_back(Frame(Vec3_t(0.5 * i, 0.0, 1.6), i, num_landmarks));

            setObservations(frames, gt_landmarks);

            //noise landmarks using triangulation
            // setNoiseLandmarks(noise_landmarks, frames, gt_landmarks);
            noise_landmarks.reserve(gt_landmarks.size());
            for(const auto& landmark : gt_landmarks)
                noise_landmarks.push_back(Landmark(Vec3_t(0.0, 0.0, 10.0), landmark.id_));
        }
        ...
        if(!optimizer.setupLocalBA(frames, noise_landmarks))
            break;

        // if(!optimizer.setupPoseOptimization(frames))
        //     break;

        //structure only ba
        // optimizer.doStructureOnlyBA(30);
        ...
        while(count < num_iter)
        {
            ...
            //publish frame poses
            publishOptimalPoses(optimizer, tf_broadcaster, num_frames);
            for(const auto& frame : frames)
                publishFramePose(frame.pose_w2c_, tf_broadcaster, "GT", frame.id_);
        ...
        //         if(vtx)
        //         {
        //             g2o::SE3Quat se3 = vtx->estimate();
        //             Mat44_t pose = Mat44_t::Identity();
        //             pose.block<3,3>(0,0) = se3.rotation().toRotationMatrix();
        //             pose.block<3,1>(0,3) = se3.translation();\
        //             tmp_poses.push_back(pose.inverse());
        //         }
        //     }
        // }
        ...
        // for(int i = 0; i < num_frames; ++i)
        // {
        //     frames[i].pose_w2c_ = tmp_poses[i];
        //     frames[i].pose_c2w_ = tmp_poses[i].inverse();
        //     frames[i].pose_w2c_noise_ = tmp_poses[i];
        //     frames[i].pose_c2w_noise_ = tmp_poses[i].inverse();
        // }
