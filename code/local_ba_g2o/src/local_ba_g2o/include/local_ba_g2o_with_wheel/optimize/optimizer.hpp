...
class Optimizer
{
    public:
        ...
        bool setupLocalBA(std::vector<Frame>& frames, std::vector<Landmark>& landmarks)
        {
            /*--------------------------------------------------------------*/
            // optimizer 설정
            ...
            for(auto& frame : frames)
            {
                ...
                {
                    g2o::SE3Quat camera_pose(frame.pose_c2w_noise_.block<3,3>(0,0), frame.pose_c2w_noise_.block<3,1>(0,3));
                    camera_poses_.push_back(camera_pose);
                }
            }
            ...
            // vertex_id += landmarks.size();
            /*--------------------------------------------------------------*/
            // // wheel odometry edge 추가
            // for(int i = 0; i < frames.size() - 1; i++)
            // {
            //     Mat44_t relative_pose = frames[i+1].pose_c2w_ * frames[i].pose_c2w_.inverse();
            //     g2o::SE3Quat relative_pose_se3(relative_pose.block<3,3>(0,0), relative_pose.block<3,1>(0,3));

            //     g2o::EdgeSE3Expmap* e = new g2o::EdgeSE3Expmap();
            //     e->setVertex(0, dynamic_cast<g2o::OptimizableGraph::Vertex*>(
            //         optimizer_.vertices().find(i)->second));
            //     e->setVertex(1, dynamic_cast<g2o::OptimizableGraph::Vertex*>(
            //         optimizer_.vertices().find(i + 1)->second));
            //     e->setMeasurement(relative_pose_se3);
            //     Mat66_t information = Mat66_t::Identity() ;
        ...
        bool setupPoseOptimization(std::vector<Frame>& frames)
        {
            /*--------------------------------------------------------------*/
            // optimizer 설정
            std::string solverName;
