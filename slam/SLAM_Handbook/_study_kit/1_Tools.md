...
| 목차 생성 | `tools/init_contents.py` (PyMuPDF) | **PDF 북마크에서 `0_Contents.md` 자동 생성** — 손으로 옮기지 않는다 |
| 참고문헌 | `tools/build_refs.py` (PyMuPDF) | **References 를 `번호 → 서지정보` 로 추출** → `refs.json` |
| 보충 설명 | `tools/asides/*.md` | **aside 카드(포스트잇)** — 본문 흐름을 끊지 않고 배경 지식을 채운다 |
...
원본 PDF (ref/)
        │
        ├── init_contents.py --write        → _study_kit/0_Contents.md   (한 번)
        ├── build_refs.py --write           → _study_kit/refs.json       (한 번)
        ├── extract_figures.py --chapter N --book-pages A-B --out <챕터>/images
        ▼
챕터 .md (원본, 손으로 작성)
   +  images/*.png            (PDF에서 추출)
   +  tools/widgets/*.html    (Canvas 위젯)
   +  tools/asides/*.md       (aside 카드 — 전역)
   +  refs.json               (인용 [568] 자동 확장)
...
python3 _study_kit/tools/init_contents.py --write    # 0_Contents.md 의 AUTO-TOC 구간 갱신
python3 _study_kit/tools/init_contents.py --dump     # 북마크 원본 (구조 확인용)
```

PDF 북마크에서 만든다. **인쇄된 목차를 `pdftotext` 로 파싱하는 것보다 정확하다** —
북마크는 조판이 아니라 문서 구조에서 나오므로 하이픈이 사라지거나 합자가 깨지지 않는다
(`3_Pitfalls.md` A13).

번호 형태로 층위를 판정한다 (북마크 레벨은 책마다 제각각이라 믿지 않는다).

| 북마크 | 결과 |
|---|---|
| `Part I Foundations of SLAM` | `## Part I. Foundations of SLAM` |
| `1 Factor Graphs for SLAM` | `### 1장. Factor Graphs for SLAM (p.19)` |
| `1.1 Visualizing SLAM …` | `- 1.1 Visualizing SLAM … (20)` |
| `I Prelude` | `### Prelude I (p.3)` |
| `Epilogue` | `### Epilogue (p.548)` |
| `References` · `Author index` | 건너뜀 |

`AUTO-TOC:START` ~ `END` 사이만 갱신하므로 그 바깥에 쓴 것은 보존된다.
**북마크가 없는 PDF 면 이 도구는 그렇다고 알려 주고 멈춘다** — 그때만 손으로 옮긴다.
...
python3 _study_kit/tools/build_refs.py --show 568 1212                # 조회
```

서베이 성격의 장은 **인용 자체가 본문**이다. `refs.json` 이 있으면 노트에 원문 그대로
`[568]` 이라고만 적어도 `build_html.py` 가 저자·연도·제목을 카드로 붙인다.

빠진 번호가 있으면 경고한다. 번호가 1..N 으로 연속이어야 정상이다.

## aside 카드 (포스트잇)

책이 설명 없이 전제하는 배경을 **본문 흐름을 끊지 않고** 채운다. 본문에는 점선 밑줄만
남고, 클릭하면 오른쪽 패널에서 전문이 열린다.

```markdown
이 문제는 [[map-estimation]] 으로 푼다.        ← 전역 카드 참조
[[se3|SE(3) 위에서]] 정의된다.                  ← 표시할 말을 따로 줄 때
ScanContext[568] 가 이 방식이다.                ← 인용은 자동으로 카드가 된다
```

정의는 두 곳 중 하나에 둔다. **작성법과 판단 기준은 `tools/asides/_GUIDE.md`.**

| 위치 | 언제 |
|---|---|
| `tools/asides/이름.md` | 여러 챕터가 함께 쓰는 개념 (SE(3), MAP 추정 …) |
| 챕터 `.md` 안 `<!--aside:이름 제목-->…<!--/aside-->` | 그 장에서만 쓰는 것 |

빌드하면 무엇이 실렸는지, 무엇이 비었는지 알려 준다.
...
- 참조된 카드만 HTML 에 실린다. 전역 카드가 100개여도 파일이 부풀지 않는다
- 정의가 없어도 **빌드는 성공**한다. 회색 점선으로 표시되므로 나중에 채우면 된다
- 카드는 화면 밖에 미리 렌더된 뒤 열 때 패널로 **이동**한다. `display:none` 으로
  숨기면 MathJax 가 수식 크기를 0 으로 잡기 때문이다 — 고칠 때 이 점을 건드리지 마라
