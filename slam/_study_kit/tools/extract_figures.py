...
CAPTION_SEP = CFG["caption_sep"]       # 비볼드 책의 판정: "colon" | "none"
CAPTION_SIZE = CFG["caption_size"]     # (min, max) pt 또는 None
...
def caption_lines(page):
    """텍스트 줄을 (bbox, text, size) 로. size 는 줄 첫 span 의 글자 크기(pt).

    text_lines() 와 달리 글자 크기를 함께 준다. 책 안에 다른 문서가 이미지로
    삽입돼 있으면 그 안의 캡션이 함께 잡히는데, 크기로 걸러 내야 하기 때문이다.
    """
    out = []
    for blk in page.get_text("dict")["blocks"]:
        if blk.get("type") != 0:
        ...
        for line in blk["lines"]:
            txt = "".join(s["text"] for s in line["spans"]).strip()
            if txt:
                out.append((line["bbox"], txt, line["spans"][0]["size"]))
    return out


def size_ok(size):
    """라벨 글자 크기가 caption_size 범위 안인가 (설정이 없으면 항상 참)."""
    if not CAPTION_SIZE:
        return True
    return CAPTION_SIZE[0] <= size <= CAPTION_SIZE[1]
...
def caption_regex(chapter):
    ...
    장 번호는 숫자뿐 아니라 글자도 된다. 부록(Figure A.1)과, 파트 도입부를
    로마 숫자로 매기는 책(Figure I.2 · Figure II.1)이 여기 해당한다.
    """
...
def find_captions(page, chapter):
    ...
    # 무엇을 구분자로 볼지는 kit.conf 의 caption_sep 이 정한다.
    #
    #   colon : "Figure 3: 설명"  — 라벨 뒤에 콜론이나 마침표가 온다
    #   none  : "Figure 3.2 설명" — 구분자 없이 공백 뒤 대문자로 설명이 시작한다
    #
    # none 이 필요한 이유: 이 규약을 쓰는 책은 본문 언급도 "Figure 1.6 shows …"
    # 처럼 같은 모양이라 구분자로는 못 가른다. 대신 본문 언급은 예외 없이
    # 소문자 동사나 구두점(")." · "," · "(b)")으로 이어지므로, 설명이 대문자로
    # 시작하는지만 보면 갈린다.
    # (?-i:[A-Z]) 로 이 부분만 대소문자를 구분한다. re.I 를 그냥 두면 [A-Z] 가
    # 소문자까지 받아서 본문 언급을 하나도 못 거른다.
    #
    # `|\s*$` — 라벨만 있고 설명이 다음 줄로 넘어간 캡션(A3 의 변형)도 받는다.
    # 본문 언급은 라벨 뒤에 반드시 뭔가가 이어지므로("Figure 1.1 a dense method…")
    # 라벨 단독 줄을 캡션으로 봐도 안전하다.
    #
    # `\s+\(` — "Figure I.3 (a) In landmark-based …" 처럼 서브패널 라벨로 시작하는
    # 캡션. 본문 언급도 괄호를 쓰지만("Figure 3.6(g)-(l) show …") 그쪽은 번호에
    # 공백 없이 붙으므로, 공백 유무로 갈린다.
    tail = (r"(?:\s*[:.]\s+\S|\s*$)" if CAPTION_SEP == "colon"
            else r"(?:\s+(?-i:[A-Z])|\s+\(|\s*$)")
    line_re = re.compile(r"^(Figure|Table)\s*(\d+|[A-Za-z]+)(?:\.(\d+))?" + tail, re.I)
    for (x0, y0, x1, y1), txt, size in caption_lines(page):
        if not size_ok(size):
            continue
        m = line_re.match(txt)
        if not m:
            continue
        # 장 번호가 글자면 대문자로 정규화한다 (부록 a.1 → A.1, 로마 i.2 → I.2)
        num = m.group(2) if m.group(2).isdigit() else m.group(2).upper()
        if CAPTION_STYLE == "chapter":
            if not m.group(3) or (chapter and num != str(chapter)):
                continue
            label = "%s %s.%s" % (_kind(m), num, m.group(3))
        else:
            if m.group(3):                      # flat 인데 점 번호면 다른 규약이다
                continue
            label = "%s %s" % (_kind(m), num)
...
def main():
    ...
    ap.add_argument("--chapter",
                    help="챕터 번호 (예: 6). 부록은 글자로 준다 (예: A). "
                         "파트 도입부를 로마 숫자로 매기는 책은 I·II·III 도 된다. "
    ...
    if args.chapter and not re.fullmatch(r"\d+|[A-Za-z]+", args.chapter):
        ap.error("--chapter 는 숫자(6) 또는 글자(A · I · III) 여야 한다: %r" % args.chapter)
    if args.chapter:
