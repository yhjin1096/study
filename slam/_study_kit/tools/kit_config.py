...
kit.conf 형식 (key = value, '#' 이후는 주석):
    ...
    caption_sep   = colon            # 비볼드 책의 캡션 판정 방식 (아래 설명)
    caption_size  = 8.0-10.5         # 캡션 라벨의 글자 크기 범위 (pt). 생략하면 무제한

`caption_bold = no` 인 책은 폰트로 캡션과 본문 언급을 구분할 수 없다.
그때 무엇을 근거로 판정할지가 `caption_sep` 이다.

    colon (기본)  "Figure 3: 설명"  — 라벨 뒤에 콜론이나 마침표가 반드시 온다
    none          "Figure 3.2 설명" — 구분자 없이 공백 뒤 **대문자**로 설명이 시작한다

`none` 은 본문 언급("Figure 1.6 shows how …", "Figure 3.7).")을 소문자·구두점으로
걸러 낸다. SLAM Handbook 에서 이 규칙으로 캡션 197 건이 전부 맞고 본문 언급 29 건이
전부 배제됐다.

`caption_size` 는 책 안에 **다른 문서가 이미지로 삽입된** 경우에 필요하다. 그 안에
인쇄된 캡션(작은 글자)이 함께 잡히기 때문이다. 조사법은 3_Pitfalls.md A14.
    ...
    heading_font = CMBX|CMBXTI        # 절 제목 폰트 이름 정규식 (아래 설명)

`heading_size` 는 "제목이 본문보다 크다"를 전제한다. 그렇지 않은 판형이 있다.
LaTeX book 클래스는 \section 을 **본문과 같은 크기의 볼드**로 조판하는데,
그런 책은 크기로 절 제목을 하나도 못 잡는다. 그때 `heading_font` 에 제목
폰트 이름의 정규식을 주면 크기 대신 그것으로 판정한다.

    SLAM Handbook : 절 CMBX10 10pt · 하위 절 CMBXTI10 10pt · 본문 CMR10 10pt
                    → heading_font = CMBX
"""
import glob
import os
...
DEFAULTS = {
    ...
    "caption_sep": "colon",       # 비볼드 책의 캡션 판정: "colon" = 라벨 뒤 :/. 필수
                                  #                        "none"  = 구분자 없이 대문자 설명
    "caption_size": None,         # (min, max) pt — 라벨 글자 크기 범위. None 이면 무제한
    ...
    "heading_font": None,  # check_toc.py — 절 제목 폰트 이름 정규식. 주면 크기 대신 이걸로 판정
}
    ...
    if os.path.exists(path):
        for raw in open(path, encoding="utf-8"):
            ...
            if k == "page_offset":
                ...
                if v not in ("colon", "none"):
                    raise SystemExit(f"caption_sep 은 colon 또는 none: {v!r}")
                cfg[k] = v
            elif k == "caption_size":
                cfg[k] = _pair(v, float)
            elif k == "pdf":
