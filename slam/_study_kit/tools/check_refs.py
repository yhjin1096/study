...
def real_captions():
    ...
    **판정 로직은 extract_figures.find_captions() 를 그대로 쓴다.** 예전에는 이 파일이
    같은 로직을 따로 구현하고 있었는데, 그러면 캡션 규약이 특이한 책을 만날 때마다
    두 곳을 고쳐야 하고 한쪽만 고치면 조용히 어긋난다 (실제로 그랬다 —
    caption_sep·caption_size 를 extract_figures 에만 넣었더니 이쪽이 0개를 수집했다).
    """
    doc = fitz.open(PDF)
    found = set()
    for pno in range(doc.page_count):
        for label, _y in ef.find_captions(doc[pno], None):
            found.add(label)
    doc.close()
    return found
